...
이 문서는 Dicoshot SDK를 처음부터 Maven Central에 배포하는 전 과정을 다룹니다. 신규 컨트리뷰터가 동일한 환경을 재현하거나, 다음 버전을 릴리스할 때 참고할 수 있도록 작성되었습니다.
...
[사용자가 받기]
   │
   └─→ implementation 'io.github.dicoshot:dicoshot-...' 로 의존성 추가
```

핵심 개념:
- **group ID**: `io.github.dicoshot` — 도메인 형식의 namespace. GitHub username 기반
- **artifact ID**: 모듈 이름 (예: `dicoshot-spring-boot-3-starter`)
...
def publishableProjects = [
        'dicoshot-core',
        'dicoshot-spring-boot-3-starter',
        'dicoshot-spring-boot-4-starter'
]
...
다음 디렉토리들이 있어야 합니다:
- `dicoshot-core/0.1.0/`
- `dicoshot-spring-boot-3-starter/0.1.0/`
- `dicoshot-spring-boot-4-starter/0.1.0/`

각 디렉토리 안의 파일:
```
dicoshot-core-0.1.0.jar              # 본 artifact
dicoshot-core-0.1.0.jar.asc          # GPG 서명
dicoshot-core-0.1.0-sources.jar      # 소스 jar
dicoshot-core-0.1.0-sources.jar.asc
dicoshot-core-0.1.0-javadoc.jar      # javadoc jar
dicoshot-core-0.1.0-javadoc.jar.asc
dicoshot-core-0.1.0.pom              # 메타데이터
dicoshot-core-0.1.0.pom.asc
dicoshot-core-0.1.0.module           # Gradle module metadata
```
...
확인 방법:
```bash
curl -I https://repo.maven.apache.org/maven2/io/github/dicoshot/dicoshot-core/0.2.0/dicoshot-core-0.2.0.jar
```
...
```bash
git tag -a v0.1.0 -m "Dicoshot v0.1.0 — initial Maven Central release"
...
3. **Release title**: `Dicoshot v0.1.0`
...
```markdown
[![Maven Central](https://img.shields.io/maven-central/v/io.github.dicoshot/dicoshot-spring-boot-3-starter)](https://central.sonatype.com/artifact/io.github.dicoshot/dicoshot-spring-boot-3-starter)
```
...
```bash
./gradlew clean publishToMavenLocal
ls ~/.m2/repository/io/github/dicoshot/dicoshot-core/0.2.0/
```
...
```bash
git tag -a v0.2.0 -m "Dicoshot v0.2.0"
